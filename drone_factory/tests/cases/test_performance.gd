...
func test_large_factory_tick_budget() -> void:
	...
	# Тик идёт 10 раз в секунду: 3 мс на тик — это 3% времени на десктопе
	# и порядка 10-15% на слабом телефоне.
	check(per_tick < 3.0, "тик фабрики из %d зданий: %.2f мс" % [world.buildings.count(), per_tick])
