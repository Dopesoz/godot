...
## Готова ли база взять именно это задание. Порту всё равно, а вот хижина
## носильщиков без дерева соглашается только на подвоз дерева себе.
func accepts_task(_item_id: StringName, _target_id: int) -> bool:
	return true


## Курьер вернулся на площадку. Порту это ничего не стоит, хижина здесь
## списывает дерево за ходку.
